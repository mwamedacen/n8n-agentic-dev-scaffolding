#!/usr/bin/env python3
"""Activate a deployed workflow on its env's n8n instance."""
import argparse
import sys
...
def _resolve_workflow_id(env_name: str, workflow_key: str, workspace: Path) -> str:
    ...
    try:
        ...
        raise SystemExit(
            ...
            f"Run `python3 <harness>/helpers/bootstrap_env.py --env {env_name}` to mint placeholder IDs."
        )
...
def main() -> None:
    ...
    print(f"Activated workflow '{args.workflow_key}' (id={wf_id}) on env '{args.env}'")
